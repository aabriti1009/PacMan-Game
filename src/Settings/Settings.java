...
public class Settings {
    ...
    static private void setDefaultSettings() {
        ...
        // Game mechanics
        newMapping.put(EParam.pacman_speed,             4.0);
        newMapping.put(EParam.pacman_starting_lives,    3);
    
        newMapping.put(EParam.ghost_speed,             3.0);
        newMapping.put(EParam.ghost_count,              4);
        newMapping.put(EParam.ghost_vuln_speed,         2.0);
        newMapping.put(EParam.ghost_vuln_val,           200);
    
        newMapping.put(EParam.food_distancing,          15);
        newMapping.put(EParam.food_points,              10);
        newMapping.put(EParam.large_food_points,        50);
        ...
        // Audio
        newMapping.put(EParam.music_volume,             80);  // 0-100 scale
        newMapping.put(EParam.sfx_volume,               100); // 0-100 scale
        newMapping.put(EParam.audio_enabled,            false);
    ...
    static public void rescaleSettings(double scalefactor) {
        mapping.put(EParam.pacman_speed,    (double)mapping.get(EParam.pacman_speed)*scalefactor);
        mapping.put(EParam.ghost_speed,     (double)mapping.get(EParam.ghost_speed)*scalefactor);
        mapping.put(EParam.ghost_vuln_speed,(double)mapping.get(EParam.ghost_vuln_speed)*scalefactor);
